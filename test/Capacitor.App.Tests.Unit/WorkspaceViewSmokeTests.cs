    ...
    public async Task WorkspaceView_resolves_all_named_controls() {
        await RunOnUiAsync(async () => {
            ...
            var names = new[] {
                ...
                "DetachButton", "ChatHost", "WorkContextHost",
            };
            foreach (var name in names)
                await Assert.That(Find<Control>(window, name)).IsNotNull().Because($"{name} should resolve");
            // ReattachButton and SessionEndedNote live inside banners that are collapsed here. A Surface
            // realises its content on first measure, so they exist in the name scope only.
            foreach (var name in new[] { "ReattachButton", "SessionEndedNote" })
