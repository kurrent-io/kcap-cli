...
public class ServerPullRequestSourceTests {
    ...
    public async Task Three_missing_reads_stop_only_that_session_and_retry_resets_it() {
        ...
        await using var source = new ServerPullRequestSource(Config.Root, Resolutions.At("https://server.test", Config.Root), ProfileOverrides.None, MachineAuth.None, TimeProvider.System,
    ...
    public async Task Sign_in_during_client_creation_cannot_publish_the_old_authenticated_client() {
        ...
        var calls = 0;
        await using var source = new ServerPullRequestSource(Config.Root, Resolutions.At("https://server.test", Config.Root), ProfileOverrides.None, MachineAuth.None, TimeProvider.System, (_, _, _, _) => {
            calls++;
