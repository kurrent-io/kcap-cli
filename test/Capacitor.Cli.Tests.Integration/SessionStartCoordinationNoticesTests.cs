...
public class SessionStartCoordinationNoticesTests : IDisposable {
    ...
    public async Task Advertises_the_capability_and_renders_returned_notices() {
        ...
        await new ClaudeHookCommand(Config.Root, Resolutions.At(_server.Url!, Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient()).Handle(new StringReader(Payload()), stdout: stdout);
