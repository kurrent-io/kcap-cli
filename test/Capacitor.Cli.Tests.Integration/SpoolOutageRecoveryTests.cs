...
public class SpoolOutageRecoveryTests : IDisposable {
    ...
    Task<int> Invoke(HttpClient client, string payload) =>
        new ClaudeHookCommand(Config.Root, Resolutions.At(_server.Url!, Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(Config.Root, Resolutions.At(_server.Url!, Config.Root), new FixedCapacitorHttpClient()))
