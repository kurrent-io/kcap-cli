...
public class PermissionRequestRecoveryTests : IDisposable {
    ...
    PermissionRequestCommand CommandAsync() {
        ...
        return new(Config.Root, profiles, new HostedAgent(null, IsRendered: true, DaemonBridge.None), sp.GetRequiredService<ICapacitorHttpClient>(), TestWatchers.For(Config.Root, profiles, sp.GetRequiredService<ICapacitorHttpClient>()), TimeProvider.System);
    }
