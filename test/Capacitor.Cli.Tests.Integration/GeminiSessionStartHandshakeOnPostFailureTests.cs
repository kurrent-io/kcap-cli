...
public class GeminiSessionStartHandshakeOnPostFailureTests : IDisposable {
    ...
    async Task<(int ExitCode, string Stdout)> RunAsync(string sessionId, string source) {
        ...
        var exit = await new GeminiHookCommand(Config.Root, Resolutions.At(_server.Url!, Config.Root), new HookClock(TimeProvider.System), Home, new FixedCapacitorHttpClient())
