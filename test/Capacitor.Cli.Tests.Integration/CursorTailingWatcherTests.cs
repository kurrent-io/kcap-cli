...
public class CursorTailingWatcherTests {
    ...
    public async Task Reactivation_ViaSessionStartHook_SpawnsAFreshWatcher() {
        ...
        var exit = await new CursorHookCommand(Config.Root, Resolutions.At(server.Url!, Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), TestWatchers.For(Config.Root, Resolutions.At(server.Url!, Config.Root), new FixedCapacitorHttpClient())).HandleCore(client, new StringReader(body), spool);
