...
public class CursorSessionStartVisibilityTests : IDisposable {
    ...
    async Task<JsonNode> RunSessionStartAndCaptureBodyAsync(string defaultVisibility) {
        ...
        var exit = await new CursorHookCommand(Config.Root, Resolutions.At(_server.Url!, Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient()).HandleCore(client, new StringReader(body), spool);
