...
public class SessionStartMemoryRedirectTests : IDisposable {
    ...
    async Task<(int ExitCode, string Stdout)> RunAsync(string sessionId) {
        ...
        var exit = await new GeminiHookCommand(Config.Root, profiles, new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, sp.GetRequiredService<ICapacitorHttpClient>(), new WatcherManager(Config.Root, profiles, sp.GetRequiredService<ICapacitorHttpClient>()))
