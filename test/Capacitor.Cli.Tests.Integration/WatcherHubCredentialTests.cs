...
public class WatcherHubCredentialTests : IDisposable {
    ...
    WatchCommand Watch(string? bearer) =>
        new(Config.Root, Resolutions.At(_server.Url!, Config.Root), TestHarnesses.Under(Home), new FixedCapacitorHttpClient(), new FixedCredentialSource(bearer), new WatcherManager(Config.Root, Resolutions.At(_server.Url!, Config.Root), new FixedCapacitorHttpClient()));
