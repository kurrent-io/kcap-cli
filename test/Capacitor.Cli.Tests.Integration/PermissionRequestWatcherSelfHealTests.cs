...
public class PermissionRequestWatcherSelfHealTests {
    ...
    public async Task SpawnsWatcher_WhenMainSessionTranscriptPresent() {
        ...
        await new PermissionRequestCommand(Root, Resolutions.At("http://localhost:0", Root), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(Root, Resolutions.At("http://localhost:0", Root), new FixedCapacitorHttpClient())).TryEnsureWatcher(sessionId, node);
