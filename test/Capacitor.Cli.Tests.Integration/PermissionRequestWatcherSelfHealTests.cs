...
public class PermissionRequestWatcherSelfHealTests {
    ...
    static readonly WatcherManager Watchers = TestWatchers.For(Root, Resolutions.At("http://localhost:0", Root), new FixedCapacitorHttpClient());
    ...
    public async Task SpawnsWatcher_WhenMainSessionTranscriptPresent() {
        ...
        await new PermissionRequestCommand(Root, Resolutions.At("http://localhost:0", Root), HostedAgent.Terminal, new FixedCapacitorHttpClient(), TestWatchers.For(Root, Resolutions.At("http://localhost:0", Root), new FixedCapacitorHttpClient())).TryEnsureWatcher(sessionId, node);
