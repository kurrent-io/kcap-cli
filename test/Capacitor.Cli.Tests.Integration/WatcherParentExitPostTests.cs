...
public class WatcherParentExitPostTests : IDisposable {
    ...
    // construction.
    WatchCommand Watch() => new(Config.Root, Resolutions.At(_server.Url!, Config.Root), TestHarnesses.Under(Home), new FixedCapacitorHttpClient(), new FixedCredentialSource(), TestWatchers.For(Config.Root, Resolutions.At(_server.Url!, Config.Root), new FixedCapacitorHttpClient()), new GitProviderRouter(), TimeProvider.System);
    ...
    public async Task ParentExit_FinalizesGeminiSubagents_BeforeSessionEnd() {
        ...
        var parent = chats.PathTo("session-2026-06-22T14-31-0a900000.jsonl");
