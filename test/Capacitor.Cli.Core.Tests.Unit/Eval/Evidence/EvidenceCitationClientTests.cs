...
/// <summary>Batches of at most 64, work_budget refs resent, a 404/409 run-fatal with nothing certified, and refs the
/// shared 90 s budget leaves dropped and counted.</summary>
public class EvidenceCitationClientTests : IDisposable {
    ...
    public async Task Seventy_refs_go_in_two_requests_and_come_back_in_request_order() {
        ...
        var outcome = await Client().CertifyAsync("tok", [.. Enumerable.Range(0, 70).Select(i => $"{EvidenceServerStub.RootSource}@{i}")], CancellationToken.None);
    ...
    public async Task A_work_budget_ref_is_resent_and_certified_later() {
        ...
        var outcome = await Client().CertifyAsync("tok", [r0, r1], CancellationToken.None);
    ...
    public async Task A_whole_request_refusal_is_scope_loss_with_nothing_certified(int status) {
        ...
        var outcome = await Client().CertifyAsync("tok", [$"{EvidenceServerStub.RootSource}@0"], CancellationToken.None);
    ...
    public async Task Refs_the_budget_leaves_are_dropped_and_counted() {
        ...
        var running = Client(time).CertifyAsync("tok", [.. Enumerable.Range(0, 3).Select(i => $"{EvidenceServerStub.RootSource}@{i}")], CancellationToken.None);
