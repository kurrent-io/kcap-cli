...
public class TitleGenerationTests {
    ...
    public async Task CleanTitle_strips_markdown_and_trailing_punctuation() {
        var result = TitleGeneration.CleanTitle("**Fix auth timeout in login flow?**");
    ...
    public async Task CleanTitle_caps_length_at_120() {
        ...
        var result = TitleGeneration.CleanTitle(longTitle);
    ...
    public async Task CleanTitle_returns_null_for_refusal_preamble(string refusal) {
        var result = TitleGeneration.CleanTitle(refusal);
    ...
    public async Task CleanTitle_accepts_valid_titles(string title) {
        var result = TitleGeneration.CleanTitle(title);
    ...
    public async Task CleanTitle_returns_null_for_empty_input() {
        var result = TitleGeneration.CleanTitle("");
    ...
    public async Task TitlePromptPrefix_is_stable_and_non_empty() {
        await Assert.That(TitleGeneration.TitlePromptPrefix).IsNotEmpty();
        await Assert.That(TitleGeneration.TitlePromptPrefix).EndsWith(". ");
    }

    [Test]
    public async Task SanitizeForLog_escapes_newlines() {
        var result = TitleGeneration.SanitizeForLog("one\ntwo\r\nthree", 200);
    ...
    public async Task SanitizeForLog_strips_control_chars_but_keeps_visible_chars() {
        var result = TitleGeneration.SanitizeForLog("a\tbc!d", 200);
    ...
    public async Task SanitizeForLog_caps_output_length() {
        ...
        var result = TitleGeneration.SanitizeForLog(raw, 64);
