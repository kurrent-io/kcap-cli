...
public class ClaudeHookInputWaitRelayTests {
    ...
    async Task<int> RunAsync(HostedAgent hosted, string eventName, HookClock? clock = null, string extraFields = "") {
        ...
        return await new ClaudeHookCommand(Config.Root, Resolutions.At("http://server.example", Config.Root), clock ?? new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), hosted, new FixedCapacitorHttpClient(), new WatcherManager(Config.Root, Resolutions.At("http://server.example", Config.Root), new FixedCapacitorHttpClient()))
