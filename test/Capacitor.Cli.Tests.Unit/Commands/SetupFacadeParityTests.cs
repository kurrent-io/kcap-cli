...
public class SetupFacadeParityTests {
    ...
    public async Task RunDiscoveryAsync_github_two_tenants_publishes_both_and_marks_loginComplete() {
        ...
        var discovered = await new SetupCommand(Config.Root, Resolutions.None(Config.Root), new RecordingBrowser(), Home, new FixedCapacitorHttpClient(), Provisioning).RunDiscoveryAsync(["--github"], forceDevice: true);
    ...
    public async Task RunDiscoveryAsync_hands_the_requested_workspace_to_the_provisioner() {
        ...
        await new SetupCommand(Config.Root, Resolutions.None(Config.Root), new RecordingBrowser(), Home, new FixedCapacitorHttpClient(), Provisioning)
    ...
    public async Task RunLoginStepAsync_loginComplete_reports_the_already_published_identity_without_a_facade_call() {
        ...
        await new TokenStore(Config.Root).SaveAsync("acme", new StoredTokens {
            AccessToken     = "tok",
        ...
        try {
            exitCode = await new SetupCommand(Config.Root, Resolutions.None(Config.Root), new RecordingBrowser(), Home, new FixedCapacitorHttpClient(), Provisioning).RunLoginStepAsync(
    ...
    public async Task RunLoginStepAsync_none_provider_commits_the_stamp_through_the_facade() {
        ...
        var exitCode = await new SetupCommand(Config.Root, Resolutions.None(Config.Root), new RecordingBrowser(), Home, new FixedCapacitorHttpClient(), Provisioning).RunLoginStepAsync(
