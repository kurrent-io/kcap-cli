...
public class ShutdownTranscriptSpoolTests {
    ...
    /// <summary>The spooled tail carries its commits, since its lines are read as observed on replay.</summary>
    [Test]
    public async Task build_batch_carries_observed_commits_only_when_observing() {
        ...
        var not       = System.Text.Json.Nodes.JsonNode.Parse(WatchCommand.BuildTranscriptSpoolBatch(Sid, null, "claude", ["{\"k\":1}"], [0]))!;
    ...
    public async Task shutdown_spools_only_the_undelivered_tail() {
        ...
        var result = await Watch.SpoolUndeliveredTranscriptTailAsync(
            spool, transcriptPath, Sid, agentId: null, vendor: "kiro", linesProcessed: 2, commits: CommitObservation.None, ct: CancellationToken.None);
    ...
    public async Task shutdown_tail_exceeding_cap_marks_needs_import() {
        ...
        var result = await Watch.SpoolUndeliveredTranscriptTailAsync(
            spool, transcriptPath, Sid, agentId: null, vendor: "kiro", linesProcessed: 0, commits: CommitObservation.None, ct: CancellationToken.None);
    ...
    public async Task shutdown_skips_spooling_when_the_cursor_session_is_quarantined() {
        ...
        var spool  = new TranscriptSpool(spoolDir, time: TimeProvider.System);
        var result = await Watch.SpoolUndeliveredTranscriptTailAsync(
            spool, transcriptPath, sid, agentId: null, vendor: "cursor", linesProcessed: 0, commits: CommitObservation.None, ct: CancellationToken.None);
    ...
    public async Task shutdown_missing_transcript_file_is_a_noop() {
        ...
        var result = await Watch.SpoolUndeliveredTranscriptTailAsync(
            ...
            linesProcessed: 0, commits: CommitObservation.None, ct: CancellationToken.None);
