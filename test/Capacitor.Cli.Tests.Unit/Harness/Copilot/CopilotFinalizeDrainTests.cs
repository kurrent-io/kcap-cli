...
public class CopilotFinalizeDrainRunTests : IDisposable {
    ...
    public async Task DeliversShutdown_WhenAlreadyPresent() {
        ...
        await new CopilotFinalizeDrainCommand(Config.Root, TestWatchers.For(Config.Root, Resolutions.At(_server.Url!, Config.Root), new FixedCapacitorHttpClient())).RunAsync(
            sessionId, path,
    ...
    public async Task DeliversShutdown_WhenWrittenAfterDelay() {
        ...
        var runTask = new CopilotFinalizeDrainCommand(Config.Root, TestWatchers.For(Config.Root, Resolutions.At(_server.Url!, Config.Root), new FixedCapacitorHttpClient())).RunAsync(
            sessionId, path,
            pollBudget: TimeSpan.FromSeconds(5),
            pollInterval: TimeSpan.FromMilliseconds(100)
        );
