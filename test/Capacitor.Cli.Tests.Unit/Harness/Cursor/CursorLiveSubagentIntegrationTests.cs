...
public class CursorLiveSubagentIntegrationTests {
    ...
    sealed class Fixture : IDisposable {
        ...
        public Task<int> HandleAsync(string sessionId, string eventName, string? transcriptPath, string extraFields = "") =>
            new CursorHookCommand(Config, Resolutions.At("http://localhost", Config), new HookClock(TimeProvider.System), _home, TestHarnesses.Under(_home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(Config, Resolutions.At("http://localhost", Config), new FixedCapacitorHttpClient())).HandleCore(
                Client,
