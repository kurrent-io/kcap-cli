...
public class CursorGuardWiringTests {
    ...
    WatchCommand? _watch;
    WatchCommand Watch => _watch ??= new(Config.Root, Resolutions.None(Config.Root), TestHarnesses.Under(Home), new FixedCapacitorHttpClient(), new FixedCredentialSource(), new WatcherManager(Config.Root, Resolutions.None(Config.Root), new FixedCapacitorHttpClient()));
