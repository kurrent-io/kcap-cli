...
public class CursorWatcherSpawnTests {
    ...
    CursorHookCommand Hook(ConfigRoot root) => new(root, Resolutions.At("http://s", root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(root, Resolutions.At("http://s", root), new FixedCapacitorHttpClient()));
    ...
    public async Task Deferred_spool_drain_delivering_a_spooled_subagent_start_spawns_the_child_watcher() {
        ...
        try {
            ...
            startFails = true;
            await new CursorHookCommand(Config.Root, Resolutions.At("http://s", Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(Config.Root, Resolutions.At("http://s", Config.Root), new FixedCapacitorHttpClient())).HandleCore(
                client,
