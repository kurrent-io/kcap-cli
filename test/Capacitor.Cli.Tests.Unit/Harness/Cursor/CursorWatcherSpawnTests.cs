...
public class CursorWatcherSpawnTests {
    ...
    CursorHookCommand Hook(ConfigRoot root) => new(root, Resolutions.At("http://s", root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient());
