...
/// <summary>
/// The precedence-ordered per-session Cursor watcher spawn.
/// <see cref="CursorHookCommand.ShouldSpawnWatcher"/> is pure (no I/O); the
/// <see cref="CursorHookCommand.MaybeSpawnWatcherAsync"/> tests substitute the spawner, so no real
/// OS process is ever launched.
/// </summary>
public class CursorWatcherSpawnTests {
    ...
    CursorHookCommand Hook(ConfigRoot root, IWatcherSpawner spawner) =>
        new(root, Resolutions.At("http://s", root), new HookClock(TimeProvider.System), Home,
            ...
            TestWatchers.For(root, Resolutions.At("http://s", root), new FixedCapacitorHttpClient(), spawner), router: new GitProviderRouter());
    ...
    public async Task Spawn_is_suppressed_when_session_is_quarantined() {
        ...
        await Hook(Config.Root, spawner).MaybeSpawnWatcherAsync(sid, "/tmp/qsid.jsonl", cwd: null, eventName: "sessionStart", isSubagentChild: false);
    ...
    public async Task Spawn_is_suppressed_for_sessionEnd() {
        var sid     = NewSessionId();
        var spawner = new FakeWatcherSpawner();
        await Hook(Config.Root, spawner).MaybeSpawnWatcherAsync(sid, "/tmp/x.jsonl", cwd: null, eventName: "sessionEnd", isSubagentChild: false);
        await Assert.That(spawner.Keys).IsEmpty();
    }
    ...
    public async Task Spawn_is_suppressed_for_a_correlated_child() {
        var sid     = NewSessionId();
        var spawner = new FakeWatcherSpawner();
        await Hook(Config.Root, spawner).MaybeSpawnWatcherAsync(sid, "/tmp/x.jsonl", cwd: null, eventName: "sessionStart", isSubagentChild: true);
        await Assert.That(spawner.Keys).IsEmpty();
    }
    ...
    public async Task Spawn_is_suppressed_when_transcript_path_is_empty() {
        var sid     = NewSessionId();
        var spawner = new FakeWatcherSpawner();
        await Hook(Config.Root, spawner).MaybeSpawnWatcherAsync(sid, "", cwd: null, eventName: "sessionStart", isSubagentChild: false);
        await Assert.That(spawner.Keys).IsEmpty();
    }
    ...
    public async Task NonQuarantined_toplevel_spawn_invokes_the_watcher_manager_keyed_on_the_session_id() {
        var sid     = NewSessionId();
        var spawner = new FakeWatcherSpawner();
        await Hook(Config.Root, spawner).MaybeSpawnWatcherAsync(sid, "/tmp/x.jsonl", cwd: null, eventName: "sessionStart", isSubagentChild: false);
    ...
    public async Task Deferred_spool_drain_delivering_a_spooled_subagent_start_spawns_the_child_watcher() {
        ...
        await Assert.That(spawner.Keys).IsEmpty();                // so NO watcher yet
        ...
        await Assert.That(spawner.Keys).IsEquivalentTo([$"{parent}-{child}"]);
    }
    ...
    public async Task Permanently_dropped_subagent_start_gates_all_child_transcript_delivery_forever() {
        ...
        await Assert.That(spawner.Keys).IsEmpty();      // never acked -> no child watcher ever
