...
public class AntigravitySessionStartMemoryTests {
    ...
    AntigravityHookCommand Hook(string serverUrl = "https://example.test") =>
        new(Config.Root, Resolutions.At(serverUrl, Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(Config.Root, Resolutions.At(serverUrl, Config.Root), new FixedCapacitorHttpClient()));
    ...
    public async Task A_non_PreInvocation_event_writes_nothing_and_exits_zero() {
        var sw   = new StringWriter();
        var code = await new AntigravityHookCommand(Config.Root, Resolutions.At("https://example.test", Config.Root), new HookClock(TimeProvider.System), Home, TestHarnesses.Under(Home), HostedAgent.Terminal, new FixedCapacitorHttpClient(), new WatcherManager(Config.Root, Resolutions.At("https://example.test", Config.Root), new FixedCapacitorHttpClient())).Handle(["--antigravity", "Stop"], new StringReader("{}"), sw);
