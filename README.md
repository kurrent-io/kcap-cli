...
- **First-run notice** — the session that runs `kcap setup` has no hooks, skills or MCP servers, because an agent reads those when it starts: it is not recorded, and it cannot run the guided tour. Setup leaves a one-shot marker, and the first session that starts with hooks in place opens with a short block saying recording is on and offering the tour — the session that can say it truthfully. It is armed only when setup installed something, claimed by an atomic rename so several agents starting at once deliver it once between them, and suppressed by `kcap config set disable_first_run_notice true` (which leaves the marker alone, so re-enabling before that first session still delivers it).
