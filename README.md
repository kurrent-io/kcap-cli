...
Requires `kcap login` **and a running daemon with this repo checked out**. Discovery filters by the effective reviewer vendor before tie-breaking. Claude certification additionally uses structured CLI/launcher-policy capability metadata; borrowed-checkout Claude review remains disabled, so Claude reviews use owned worktrees.
