...
A start or round call can also hit two settlement-layer coded 409s — `flow_settlement_busy` and `reviewer_launch_incarnation_superseded` — that resolve quickly against the server's own reconciliation. The CLI transparently auto-retries these a few times before surfacing anything, so they're normally invisible to the caller.
