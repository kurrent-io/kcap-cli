...
/// </summary>
internal sealed class MachineCredentials(MachineCredential credential) : ICredentialSource {
    ...
    async Task<CredentialState> MintAsync(string? refused, CancellationToken ct) {
        var minted = await MachineTokenProvider.GetTokenAsync(credential, refused, ct);
