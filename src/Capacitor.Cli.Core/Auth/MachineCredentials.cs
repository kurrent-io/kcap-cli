...
/// </summary>
internal sealed class MachineCredentials(WorkOSClient workos, MachineCredential credential) : ICredentialSource {
    ...
    async Task<CredentialState> MintAsync(string? refused, CancellationToken ct) {
        var minted = await MachineTokenProvider.GetTokenAsync(workos, credential, refused, ct);
