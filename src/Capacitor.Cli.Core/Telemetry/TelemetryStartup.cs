...
/// which server decides the organization group, which host the join's first hop targets, and
/// whether the desktop app spawned it.
///
...
/// </summary>
public sealed record TelemetryStartup(
        string Command, string? ServerUrl, string SignupUrl, bool Suppressed, bool Debug) {
    /// <summary>
    ...
    public static TelemetryStartup FromEnvironment(string command, string? serverUrl, string signupUrl) {
        ...
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable(SpawnNoTelemetryVar)))
            return new TelemetryStartup(command, serverUrl, signupUrl, Suppressed: false, debug);

        Environment.SetEnvironmentVariable(SpawnNoTelemetryVar, null);

        return new TelemetryStartup(command, serverUrl, signupUrl, Suppressed: true, debug);
    }
}
