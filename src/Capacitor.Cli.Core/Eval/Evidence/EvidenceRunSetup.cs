...
public sealed class EvidenceRunSetup : IAsyncDisposable {
    ...
    /// <summary>The run's reporting question, chosen over the catalog's order as the server chooses it.</summary>
    public string? ReportingQuestionId { get; init; }

    // One answered fetch per strategy per run; null records a strategy the server answered without a built view.
