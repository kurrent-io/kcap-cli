...
public static class CuratedBlock {
    ...
    public static string Splice(string content, string? renderedBlock) {
        ...
        if (loc is null) {
            ...
            if (lines.Count > 0 && !(lines is [{ Length: 0 }]) && lines[^1].Trim().Length != 0)
                lines.Add("");
    ...
    static (int start, int end)? Locate(string[] lines) {
        ...
        return starts switch {
            0 when ends == 0                => null,
            1 when ends == 1 && start < end => (start, end),
            _                               => throw new CuratedBlockException($"Malformed managed block: found {starts} start and {ends} end marker(s). Fix the file by hand and re-run.")
        };
    }
}
