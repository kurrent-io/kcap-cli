...
internal static class WizardComposition {
    ...
    internal static WizardGraph BuildGraph(WizardGraphOptions options) {
        ...
        signIn.Completed += () => _ = AdvanceAfterHoldAsync(wizard, options.Time, options.ShutdownToken);

        return new WizardGraph(wizard, auth, steps, import);
    }

    static async Task AdvanceAfterHoldAsync(OnboardingViewModel wizard, TimeProvider time, CancellationToken ct) {
        try {
        ...
        wizard.TryAdvanceFrom(WizardStepId.SignIn);
    }
