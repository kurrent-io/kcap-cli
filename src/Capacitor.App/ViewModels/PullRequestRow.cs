...
public sealed record PullRequestRow(string Id, string Title, string Detail, string? Body, string? Hunk, string? Url,
    ...
    bool IsReviewer = false, string? ActorKind = null, PullRequestStatus? Status = null, bool ReRequested = false,
    ...
    internal long Bytes => 256 + 2L * (Id.Length + Title.Length + Detail.Length + (Body?.Length ?? 0) + (Hunk?.Length ?? 0) + (Url?.Length ?? 0) + (AvatarUrl?.Length ?? 0));
}
