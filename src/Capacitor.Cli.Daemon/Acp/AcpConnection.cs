...
/// </summary>
internal sealed class AcpRpcException(int code, string message, JsonElement? data = null) : Exception(message) {
...
/// </summary>
internal sealed class AcpConnection(Stream writeStream, Stream readStream, ILogger logger) : IAsyncDisposable {
    ...
    public async Task RunAsync(CancellationToken ct) {
        using var reader = new StreamReader(readStream, Encoding.UTF8, detectEncodingFromByteOrderMarks: false, leaveOpen: true);
    ...
    async Task DispatchLineAsync(string line, CancellationToken ct) {
        ...
        try {
            ...
            logger.LogDebug(ex, "ACP: skipping unparseable line ({Length} chars)", line.Length);
        ...
        try {
            using (doc) {
                ...
                if (root.ValueKind != JsonValueKind.Object) {
                    logger.LogDebug("ACP: skipping non-object frame (kind={Kind})", root.ValueKind);
                ...
                switch (hasMethod) {
                    ...
                    case true when !hasId:
                        ...
                        return;
                    default:
                        logger.LogDebug("ACP: skipping frame with unrecognized shape"); break;
                }
            }
            ...
            logger.LogDebug(ex, "ACP: skipping frame with wrong-typed field (method.kind={MethodKind}, id.kind={IdKind})", methodKind, idKind);
        }
    }
    ...
    void HandleResponse(JsonElement idElement, JsonElement? resultElement, JsonElement? errorElement) {
        if (!idElement.TryGetInt64(out var id)) {
            logger.LogDebug("ACP: response id is not a numeric value we issued; ignoring");

            return;
        }

        if (!_pending.TryRemove(id, out var tcs)) {
            logger.LogDebug("ACP: no pending request for response id={Id}; ignoring", id);
    ...
    async Task HandleServerRequestAsync(JsonElement root, JsonElement idElement, JsonElement methodElement, CancellationToken ct) {
        ...
        if (handler is null) {
            ...
            try {
                ...
                logger.LogDebug(ex, "ACP: OnServerRequest handler threw for method={Method}", method);
        ...
        try {
            ...
            logger.LogDebug(ex, "ACP: failed to write server-request response for method={Method}; sending internal-error fallback", method);
            ...
            try {
                ...
                logger.LogDebug(fallbackEx, "ACP: internal-error fallback response also failed to write for method={Method}", method);
            }
        }
    }
    ...
    async Task WriteLineAsync(string json, CancellationToken ct) {
        ...
        try {
            await writeStream.WriteAsync(bytes, ct).ConfigureAwait(false);
