...
class PermissionRequestCommand(ConfigRoot config, ProfileContext profiles, ICapacitorHttpClient http) {
    ...
    async Task<int> HandleRenderedAgent(JsonNode node, string sessionId, TextWriter? stdout = null) {
        ...
        // The waiting verb: a lapsed credential is a value rather than a line, because Claude reads
        // this hook's stderr as its result — and the leg recovers from a 401, because the user answers
        // it by hand and the token it opened with can be refused before they do. Sent regardless of
        // that value; the agent is blocked on a decision.
        var       attempt      = await http.ForWaitAsync();
