...
if (Environment.GetEnvironmentVariable("KCAP_SKIP") is "1"
 ...
 && (args.Contains("--claude")      || args.Contains("--cursor") || args.Contains("--copilot") || args.Contains("--gemini") || args.Contains("--kiro") || args.Contains("--pi") ||
        args.Contains("--opencode") || args.Contains("--antigravity"))) {
    return 0;
}
...
string[] offlineCommands =
    ["--help", "-h", "help", "--version", "-v", "logout", "cleanup", "config", "daemon", "setup", "status", "update", "plugin", "profile", "use", "repos", "login", "ignore", "remap", "uninstall"];
...
try {
    switch (command) {
        ...
        case "import": {
            ...
            var resolveResult = ImportScopeArgs.Resolve(
                new(
                    Flags: flags,
        ...
        case "watch" when args.Length < 3:
            Console.Error.WriteLine(
                "Usage: kcap watch <sessionId> <transcriptPath> [--agent-id <agentId>] [--cwd <cwd>] [--skip-title] [--parent-pid <pid>] [--vendor claude|codex|copilot|gemini|kiro|pi|opencode|antigravity]"
            );
