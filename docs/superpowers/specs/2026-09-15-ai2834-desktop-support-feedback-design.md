...
2. **Rail footer help button**: a 22px icon button at the right end of the footer strip, before `HostedText`, drawing a circled question mark in the rail's stroke style, with `ToolTip.Tip="Help and support"` and `AutomationProperties.Name="Help and support"` set on the button itself so it overrides the footer strip's daemon-identity tooltip and has a name for assistive technology. It opens a `kcapPanel` flyout upward with "Documentation", "Report a bug…", "Send feedback…". The button and Documentation are always enabled — reaching the docs needs no server, and a user with no server resolved is the one most likely to need them. Only the two report items follow D3. On Windows and Linux this flyout is the only entry, since those builds have no menu bar; on macOS it is the same items one click closer.
...
The window needs a resolved server to post to. The two Help items and the two report items in the flyout are enabled exactly when `App.ServerHttp(profiles)` yields a client — a resolved `ServerUrl`, with or without a profile name, which includes a `KCAP_URL` override (the app parses no command-line server argument). This is deliberately a wider condition than Settings uses: Settings needs a profile name for its store; a report needs only a server and the token store.

The oracle is evaluated once, at startup, after the profile resolves and **before** `_coordinator.ShowMainWindow()`, and the resulting `Action<FeedbackCategory>?` is handed to both `AppMenuBar` and the main window's view model at that point. `AppMenuBar` retains the two report items of every menu copy it attaches (a per-window list, the way `AppMenu` retains its Settings item) and flips their `IsEnabled` from `SetFeedbackAction`; each item's click handler resolves the **currently stored** action at click time rather than closing over the one present when the menu was built, so an item enabled later invokes the action that enabled it, so a copy attached to a window that opens later reads the current action, and a copy attached earlier is updated in place rather than rebuilt (rebuilding an exported item drops the AppKit registration until the window next becomes key). The retained pair is removed on that window's `Closed`, so a process that opens and closes windows for hours keeps only the live copies and a later `SetFeedbackAction` touches only those.
...
- `_feedbackWindow` field. A second open while the window exists restores it from minimised, activates it, and switches the category to the one asked for **unless a send is in flight** (the frozen snapshot is not touched). If the window is showing a `Sent` confirmation, the second open starts a new blank report with the requested category and the already-minted next id — the same transition Send Another makes — so "Report a Bug…" always lands on a bug form.
...
**Composition.** One function, `FeedbackMessageComposer.Compose(userText, trailer)` (Cli.Core), returns the wire message: the trimmed user text, a blank line, and one trailer line, `Sent from Kurrent Capacitor Desktop {app version} · daemon {name} {version}`. The trailer is built by `FeedbackTrailer.Build(appVersion, daemonName, snapshotVersion, cliVersion)` (Capacitor.App), which owns the selection rule; the composer only joins. The inputs have one source each: the app version is `CapacitorVersion.CurrentDisplay()`; the daemon name is `IDaemonClientService.DaemonName` (always present); the daemon version is the version of the latest daemon snapshot the app has observed (what `MainWindowViewModel.VersionDisplay` shows), else the installed CLI version from `DaemonLifecycleController.CliVersion` rendered as `cli {version}` when no snapshot was ever observed, else the literal `version unknown`. A daemon that crashed after being observed therefore reports its last known version, which is the one the bug is about.

**When the trailer is read.** The view model recomputes the trailer — and with it the hint line and the length allowance — from the live context while the form is editable and no snapshot is bound. Pressing Send freezes the trailer into the snapshot with the category and text. While a snapshot is bound (after a refusal, with content unchanged), the hint shows the frozen trailer and an unchanged retry re-sends it unchanged, even if a daemon snapshot arrived in between; the disclosed attachment and the sent attachment are always the same bytes. An edit that mints a new id (D4 below) also releases the trailer, which is then recomputed from the current context. A version that merely arrives later is never a content change. The lane's context has no slot for the daemon identity and the server caps the context fields at 100/200 characters, so the trailer is the only place a support engineer can read which daemon the reporter was on; the hint line tells the reporter it is there. `CanSend` is true when the trimmed user text is non-empty, the **composed** message is at most 8000 characters, and no send is in flight; "{n} characters left" is 8000 minus the composed length. `client_version` and `os` ride the context as they do for the CLI.

**Snapshot and retry.** A `ClientRequestId` is minted when the window opens. Pressing Send freezes the form and binds the id to an immutable `FeedbackSubmission` snapshot (category, composed message, id, source); the call sends that snapshot. After a refusal or a failure the form unfreezes: a Send with the category and text unchanged re-sends the same snapshot and id; any edit to the text or the category — including a second entry-point click that switches the category — mints a new id. What the id buys is stable correlation, single-flight across concurrent submissions, and a replay of a success the **tenant server observed** within its retention; it is not a durable guarantee, and it is not duplicate-safe across the downstream boundary. Three duplicates are accepted and named: an unchanged retry after Plain accepted the thread but the tenant answered a non-success (the store evicts non-successes, so the sink runs again under the same id and Plain does not dedupe on it); a changed retry after any ambiguous failure, which mints a new id by rule; and a quit during the call, which loses the window-local id while the server may still finish the submission. After `Sent`, the form clears and a new id is minted for the next report.
...
- `CapacitorApiException` whose status is 401 (the final one, after `UnauthorizedRecoveryHandler`'s single rotate-and-resend) → `HomeViewModel.SignInExpiredNotice` beside a **Sign in** button. `App.OpenFeedback` injects the same sign-in action Home's banner button invokes, so the notice is actionable from the window itself; the shared server lane is not told, so the rail footer may still read Connected while an established hub stays up — accepted, since the route to recovery is on the window that discovered the lapse.
...
`source = "desktop"` reaches the tenant server and stops there: `FeedbackService` discards it today, so Plain cannot tell a desktop report from a CLI one by this field, and nothing here claims it can — the trailer in D4 is what support reads. The value is set so a later server-and-proxy change (tenant-side propagation plus a slot on the proxy submission) can forward it without touching the app; no server or proxy change is part of this design.
...
- The three duplicate cases in D4 are accepted rather than solved: solving them needs a durable pending-report store, a payload-aware server cache and downstream deduplication on the id, none of which this change adds.
- A final 401 discovered by a report does not update the shared lane's state; only the feedback window offers the sign-in route at that moment.

## Testing

- **`AppMenuBarTests`**: the Help layout becomes `Kurrent Capacitor Documentation|Changelog|-|Report a Bug…|Send Feedback…`; each new item invokes the action with its category; with no action both are disabled; `SetFeedbackAction` after `Attach` flips the items of an already-attached window without rebuilding the menu, and clicking each formerly-disabled item then invokes the new action with its category; a window attached after the action is set gets enabled items; closing an attached window removes its pair, and a later `SetFeedbackAction` updates only the live copies.
- **`FeedbackViewModelTests`** (new): `CanSend` false on empty and whitespace-only text, false when the composed message is 8001 characters and true at exactly 8000 (boundary against the largest accepted user text), false while busy; the composed message carries the trailer once with the daemon name and version; `Bug` and `Feedback` reach the fake api as their enum values; the same snapshot and id are sent twice when the content is unchanged after `TemporarilyUnavailable`, including when a daemon version arrives between the two Sends (the frozen trailer is re-sent and the hint still shows it); an edit after that releases the trailer and the next snapshot carries the new version; a new id after an edit to the text, after a category switch, and after `Sent`; a lost-success (thrown) response followed by an edit sends a new id; every `FeedbackResult` refusal maps to its sentence; a final 401 shows the sign-in notice with an enabled Sign in command that invokes the injected action; a non-401 exception leaves `CanSend` true.
...
- **`FeedbackMessageComposerTests`**: trim, separator and joining a given trailer; length arithmetic at the boundary.
- **`FeedbackTrailerTests`**: the trailer for a connected daemon, for a daemon observed then unreachable (last snapshot version), for a daemon never observed with an installed CLI (`cli {version}`), and with neither (`version unknown`).
...
- **`MainWindowSmokeTests`**: the rail footer renders the help button with the "Help and support" tooltip and automation name; Documentation is enabled with no server; the report items enable once the action is set.
