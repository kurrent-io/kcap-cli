...
- **`FeedbackMessageComposerTests`**: trim, separator and trailer; length arithmetic at the boundary; the trailer for a connected daemon, for a daemon observed then unreachable (last snapshot version), for a daemon never observed with an installed CLI (`cli {version}`), and with neither (`version unknown`).
