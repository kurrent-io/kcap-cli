...
A defect in the pass must still not take the window down with a comment. The `DocumentProcessed` handler catches at its boundary, writes the exception to stderr and stops converting; what it had not reached renders as source. That catch cannot reach the renderer, which runs later; the depth budget is what keeps the renderer's nesting limit out of an author's hands, and the view tests below render the adversarial shapes rather than only parsing them.
